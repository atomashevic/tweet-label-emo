...
ITEMS_CSV_PATH = Path("samples_18_22_consensus_items.csv")
REPORT_MD_PATH = Path("samples_18_22_consensus_report.md")
METRICS_JSON_PATH = Path("samples_18_22_consensus_metrics.json")
...
def krippendorff_alpha_nominal(item_counts: list[Counter[str]]) -> dict[str, float]:
    ...
    do = do_num / do_den
    de = (total * (total - 1) - same_total) / (total * (total - 1))
    alpha = 1 - do / de if de else math.nan
    return {"observed_disagreement": do, "expected_disagreement": de, "alpha": alpha}
...
def main() -> None:
    ...
    lines = [
        "# Samples 18-22 Consensus Analysis",
        "",
